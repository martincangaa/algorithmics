...
/* This program is used to order n elements with Quicksort combined with Insertion */
public class QuicksortInsertion {
	...
	public static void quicksort(int[] a, int maxNSubVectors) {
		quicksort(a, 0, a.length-1, 0, maxNSubVectors);
	}
	
	public static void quicksort(int[] a, int left, int right, int sizeSubVectors, int maxSizeSubVectors) {
		int i = left;
		int j = right - 1;
		int pivot;
		
		if (left < right) {
			if((right - left) <= maxSizeSubVectors) {
				insertion(a, left, right);
			}
			
			else { //if there is one element it is not necessary
			...
			if ((right - left) >= 3){ 
				...
				quicksort(a, left, i-1, i-1 - left, maxSizeSubVectors);
				quicksort(a, i+1, right, right - i+1, maxSizeSubVectors);
				}
			}
		}
	} 
